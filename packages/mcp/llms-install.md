...
The MCP server is published to npm as `agentoolbox-mcp` and communicates over **stdio**. It requires **no API key or environment variables**. The quickest install is `npx -y agentoolbox-mcp`; you can also build from source in this pnpm monorepo.
## Steps
